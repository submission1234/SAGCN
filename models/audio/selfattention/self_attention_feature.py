...
# from models.audio.vggish import vggish_params, vggish_slim
# from models.audio.vggish import vggish_input
...
def extract_features(csv_file: str, audio_dir: str, file_out_path: str):
    ...
    ds_len = len(df)
    X = np.empty((ds_len*audio_segments, AUDIO_VGGISH_EMBED), dtype=np.float32)  # 无法理解embeding
    y = np.empty(ds_len*audio_segments, dtype=np.int32)  # 这个里面为什么不是0？
    milestone = ds_len // 20  # 为什么取余20 是因为FRAMES_PER_VIDEO*VECTORS_PER_AUDIO吗？
    with tf.Graph().as_default(), tf.Session() as sess:
        # Define the model in inference mode, load the checkpoint, and
        # locate input and output tensors.
        vggish_slim.define_vggish_slim(training=False)  # 这个得改
        ...
        for idx, row in df.iterrows():
            ...
            X[counter:counter+dim] = embedding_batch  # 8612 * 128
