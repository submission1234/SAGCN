...
from definitions import (MODELS_TRAIN_OUTPUTS_FILE, MODELS_VAL_OUTPUTS_FILE, MODELS_TEST_OUTPUTS_FILE, BEST_COMBINER_MODEL)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        p = p.contiguous().view(tgt_len, bsz * self.num_heads, self.head_dim).transpose(0, 1)
        if k is not None:
        ...
        attn_weights = torch.bmm(p, k.transpose(1, 2))
...
if __name__ == '__main__':
    ...
    model_ensemble = MultiheadAttention()
