...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        h = torch.mm(input, self.W)  # 矩阵a和b矩阵相乘
        N = h.size()[0]  # 表示h的第一个维度值

        # torch.cat((tensor1,tensor2), dim)是将两个张量（tensor）拼接在一起，dim表示以哪个维度连接
        # numpy.repeat(a, repeats, axis=None)将矩阵a，按照给定的axis，将每个元素重复repeats次。
        # .view（）这个方法是对已知的矩阵进行reshape，reshape成N*N
