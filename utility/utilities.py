...
class FusionData:
    def __init__(self, features: List[Features], do_reshape: bool = True):
        ...
        for idx, feature in enumerate(self.features):
            ...
            if do_reshape:
                # if len(samples) == 21530:
                #     feature.vector_count *= 4
                #     if feature.embed_size == 320:
                #         samples = np.reshape(samples,
                #                              (len(samples) // feature.vector_count,
                #                               feature.vector_count, feature.embed_size//10))
                #     else:
                #         samples = np.reshape(samples,
                #                              (len(samples) // feature.vector_count,
                #                               feature.vector_count, feature.embed_size))
                # if len(samples) == 86120:
                #     if feature.embed_size == 320:
                #         samples = np.reshape(samples,
                #                              (len(samples) // feature.vector_count, feature.vector_count,
                #                               feature.embed_size//10))
                #     else:
                #         samples = np.reshape(samples,
                #                              (len(samples) // feature.vector_count, feature.vector_count, feature.embed_size))
                # if len(samples) == 8612:
                #     if feature.embed_size == 128:
                #         samples = np.reshape(samples,
                #                              (len(samples), feature.vector_count, feature.embed_size))
                #     else:
                #         samples = np.reshape(samples,
