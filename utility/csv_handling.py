...
FILE_TEST = 'fold1_test.csv'                 # TAU testing data
FILE_VAL = 'fold1_evaluate.csv'
...
def improve_csv(file_in_name: str, file_out_path: str, n: int = 1):
    ...
    field_names = ['audio_name', 'video_name', 'scene', 'label', 'file_name']
    ...
    for i, row in enumerate(df_in.sort_values(by=['filename']).values):
        if i % n == 0:
            ...
            file_name = row[3]
            audio_name = str(file_name) + '.wav'
            video_name = str(file_name) + '.mp4'
            # data2 = str(file_name).split('-')
            # scene = data2[0]
            # city = data2[1]
            # location = row[4]
            scene = row[2]
            ...
            print(i)
            df = pd.DataFrame(
                [[audio_name, video_name, scene, label,
                  file_name]],
                columns=field_names)
...
def generate_image_csv_from_dir_csv(csv_in: str, image_dir: str, csv_out: str):
    ...
    field_names = ['image_name', 'scene', 'label',
                   'file_name']
    ...
    for row in df_in.sort_values(by=['file_name']).values:
        ...
        while True:
            ...
            if image_name in all_images:
                ...
                # location = row[4]
                df2 = pd.DataFrame(
                    [[image_name, scene, label, file_name]],
                    columns=field_names)
...
if __name__ == '__main__':
    ...
    if generate_csvs:
        ...
        improve_csv(file_in_name=FILE_VAL, file_out_path=VAL_CSV)
