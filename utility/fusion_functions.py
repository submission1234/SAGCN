...
#                             optimizer: optim.Optimizer, train_data1: List[np.ndarray], train_data2: List[np.ndarray], train_data3: List[np.ndarray],
...
def deep_rule_func(predictions: torch.Tensor, _, model: nn.Module) -> torch.Tensor:
    ...
    Returns:
        a prediction tensor
    """
    inputs = torch.reshape(predictions, (1, 30))
    seq_len = torch.sum(torch.max(torch.abs(inputs), dim=2)[0] > 0, 1)
    return model(inputs, seq_len).squeeze(dim=0)
